...
| 2026-05-07 | Comment-to-DM automation | ManyChat-style providers | Reference, not supported | Instagram-style comment-to-DM automation is a useful growth pattern, but it mutates provider accounts, depends on platform-specific consent/limits, and is not a Main Branch provider path today. | Use as a strategy reference only. If adopted later, prefer official provider APIs, explicit operator approval, privacy-safe logs, and smoke-tested failure modes. | [#341](https://github.com/noontide-co/mainbranch/issues/341) |
