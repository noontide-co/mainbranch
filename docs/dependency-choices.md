...
| Creative media generation | Optional Ship rail; prompt/output-record first | Image generation may be used through an approved provider for the current run, but Main Branch does not claim first-class image, video, or motion generation support until provider detection, approval gates, smoke evidence, and reproducibility metadata exist. Skills may write prompts, scripts, static page visuals, CSS/JS motion, and asset indexes. Raw AI video providers remain candidates; deterministic Remotion/FFmpeg-style rendering is the preferred first motion rail. See [the creative media generation decision](../decisions/2026-05-13-creative-media-generation-rails.md). |
...
| 2026-05-13 | Creative media generation | Static image providers, deterministic render tools, and AI video providers | Prompt/output-record first; video candidates only | Visual creative tools move quickly and can become expensive, opaque, or policy-sensitive. The first durable rail is the creative brief, prompt, source context, asset index, approval state, and final file path. Static image generation can run only when a provider is configured for the current run; raw AI video stays out of core until a separate provider decision and smoke evidence exist. | Use provider-neutral prompt/output records now. Evaluate OpenAI/Google image readiness and Remotion/FFmpeg deterministic motion templates before adding `mb` provider states. | [#409](https://github.com/noontide-co/mainbranch/issues/409), [decision](../decisions/2026-05-13-creative-media-generation-rails.md) |
