...
def test_validate_accepts_matching_repo_profile(tmp_path: Path) -> None:
    _write(
        tmp_path / "core" / "offers" / "workshop" / "offer.md",
        _offer(),
    )
...
def test_validate_rejects_unknown_repo_profile(tmp_path: Path) -> None:
    _write(
        tmp_path / "core" / "operations" / "repo-topology.md",
        _repo_topology().replace(
    ...
    bad = [file for file in report["files"] if file["schema"] == "repo-topology"][0]
    assert report["ok"] is False
    assert any("profile='lightweight_website'" in error for error in bad["errors"])


def test_validate_warns_on_divergent_profile_override(tmp_path: Path) -> None:
    _write(
        tmp_path / "core" / "offers" / "workshop" / "offer.md",
        _offer(),
    )
    ...
    topology = [file for file in report["files"] if file["schema"] == "repo-topology"][0]
    assert any("overrides the role-derived profile" in warning for warning in topology["warnings"])
