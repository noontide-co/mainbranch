...
| Claude Code bridged tool or MCP | The current Claude Code session exposes a tool to the agent. | Restart Claude Code after installing or changing connectors/MCP, then ask the agent to list or perform a harmless read-only check with the tool. | Tool lists can be session-scoped. Restart after install, permission, OAuth, or scope changes before deciding the bridge is missing. |
