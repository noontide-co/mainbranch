...
The minisite is one of three site shapes `/mb-site` supports — see [`graduation.md`](../skills/mb-site/references/graduation.md) for the full ladder (lander → minisite → website → website + CMS).
...
| `og.png` | Rendered from `og.svg` post-step via [`og_render.py`](../skills/mb-site/scripts/og_render.py); not generated by the subagent |
...
Every minisite has **one** conversion endpoint — the URL the home hero CTA points to. The operator picks the kind during `/mb-site` walkthrough; the spec stays open. Stripe is one of several options; the choice depends on the offer's actual conversion goal.
...
- **No multi-endpoint sites.** One minisite, one conversion endpoint. If an offer has multiple conversion paths (lead form *and* deposit), graduate to `/mb-site → website` shape.
...
If two runs produce identical output, the soft brief was too prescriptive. Generation is supposed to surprise. See [`anti-patterns.md`](../skills/mb-site/references/anti-patterns.md) — over-prescription is the anti-pattern that kills variance.
...
**Implementation:**
- [`/mb-site` SKILL.md](../skills/mb-site/SKILL.md) — the skill that triages site shapes and routes to minisite-build.md
- [`minisite-build.md`](../skills/mb-site/references/minisite-build.md) — the operator-walkthrough flow
- [`minisite-generation-system.md`](../skills/mb-site/references/minisite-generation-system.md) — the load-bearing system prompt for the generation subagent
- [`review.md`](../skills/mb-site/references/review.md) — quality-gate steps the skill runs through before lock and before publish
- [`concept-variations.md`](../skills/mb-site/references/concept-variations.md) — parallel-on-localhost concept generation pattern
- [`anti-patterns.md`](../skills/mb-site/references/anti-patterns.md) — what NOT to bake into the system prompt
- [`graduation.md`](../skills/mb-site/references/graduation.md) — minisite → website paths + CMS bolt-on

**Atoms (deterministic external API wrappers):**
- [`domain.py`](../skills/mb-site/scripts/domain.py) — domain check + buy
- [`dns.py`](../skills/mb-site/scripts/dns.py) — CF zone + NS swap
- [`pages.py`](../skills/mb-site/scripts/pages.py) — CF Pages project + custom domain
- [`og_render.py`](../skills/mb-site/scripts/og_render.py) — SVG → PNG (1200x630)
- [`stripe.py`](../skills/mb-site/scripts/stripe.py) — Stripe payment link creation (one of several conversion-endpoint backends)
...
- **Graduation tooling:** `/mb-site graduate <new-shape>` automates minisite → website transitions
