...
Canonical bash for pulling latest vip updates at the start of /wiki. Resolves vip path via settings.local.json (no extra deps), with `~/.config/vip/local.yaml` as a PyYAML-required fallback.
