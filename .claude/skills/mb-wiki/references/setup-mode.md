...
```bash
mkdir -p ~/.mainbranch
cat > ~/.mainbranch/mb-wiki.json << 'EOF'
{
  "wiki_repo": "/absolute/path/to/mb-wiki",
  "hosting": "cloudflare",
...
**Exit:** "Wiki deployed at https://[url].pages.dev with auto-deploy enabled! Every `git push` will automatically deploy. Run `/mb-wiki configure` to personalize (name, avatar, social links, etc.)"
