...
Detailed flow for detecting the user's current working directory and configuring vip during `/mb-setup`. This is the critical first step — must happen BEFORE context gathering so essential config is saved if conversation compacts.
...
test -f ".claude/skills/mb-setup/SKILL.md" && echo "IS_VIP"
```
...
3. **If vip loaded:** Check compatibility symlinks exist (without clobbering local files):
   ```bash
   # At minimum, /mb-start must be discoverable
...
> 3. Start fresh (`/mb-setup` will create everything)"
