...
`ranked_actions`. Use GitHub activity `author_display` / `author_known` fields
when naming people. Parse the full JSON once; do not slice status output with
`head` or `sed` in the normal path.
...
- `validation.file_contracts` maps business-file shape gaps. For offer gaps,
  route to `/mb-think` and ask before editing durable offer files.
