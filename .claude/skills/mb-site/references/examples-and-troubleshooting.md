...
Usage examples and common issue fixes for `/mb-site`.
...
**Claude:** "/mb-site walks brief → site as one flow. Pick a shape:
...
Run `/mb-site` and pick "new site from scratch." The skill walks setup, then writes `~/.mainbranch/sites.json` so future invocations know where your site repo is.
...
Expect 3/3 passed (Cloudflare scopes + zone lookup + domain-check CLI). If anything's red, route to `bash .claude/skills/mb-site/scripts/setup_creds.sh`. Porkbun skipped is fine for the CF-registered path.
...
Run `/mb-site` again and trigger the review gates. The "de-AI'd" gate flags AI tells. The "in-voice" gate checks against `voice.md`. If voice.md is thin, that's the upstream fix — strengthen via `/mb-think codify`.
...
Check `<site_repo>/.mainbranch/conversion.json`. The generation subagent reads `kind` + `url` + `render` and substitutes the URL into every CTA href. If the file is missing or has `https://CONVERSION-PLACEHOLDER`, run `/mb-site` again and re-run the conversion-endpoint phase.
