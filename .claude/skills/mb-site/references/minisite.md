...
The minisite is one of three site shapes `/mb-site` supports — see [`graduation.md`](graduation.md) for the full ladder (lander → minisite → website → website + CMS).
...
| `og.png` | Rendered from `og.svg` post-step via [`og_render.py`](../scripts/og_render.py); not generated by the subagent |
...
If two runs produce identical output, the soft brief was too prescriptive. Generation is supposed to surprise. See [`anti-patterns.md`](anti-patterns.md) — over-prescription is the anti-pattern that kills variance.
...
**Implementation:**
- [`/mb-site` SKILL.md](../SKILL.md) — the skill that triages site shapes and routes to minisite-build.md
- [`minisite-build.md`](minisite-build.md) — the operator-walkthrough flow
- [`minisite-generation-system.md`](minisite-generation-system.md) — the load-bearing system prompt for the generation subagent
- [`review.md`](review.md) — quality-gate steps the skill runs through before lock and before publish
- [`concept-variations.md`](concept-variations.md) — parallel-on-localhost concept generation pattern
- [`anti-patterns.md`](anti-patterns.md) — what NOT to bake into the system prompt
- [`graduation.md`](graduation.md) — minisite → website paths + CMS bolt-on

**Atoms (deterministic external API wrappers):**
- [`domain.py`](../scripts/domain.py) — domain check + buy
- [`dns.py`](../scripts/dns.py) — CF zone + NS swap
- [`pages.py`](../scripts/pages.py) — CF Pages project + custom domain
- [`og_render.py`](../scripts/og_render.py) — SVG → PNG (1200x630)
- [`stripe.py`](../scripts/stripe.py) — Stripe payment link creation (one of several conversion-endpoint backends)
