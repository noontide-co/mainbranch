...
> **Legacy reference.** Cloudflare Pages with git auto-deploy is the V1 default for `/mb-site` (better CLI, better domain integration, deploys triggered by `git push`). This file documents the **legacy Netlify path** for pre-V1 Next.js templates that haven't migrated. Pick Netlify only if you have a specific reason to.
>
...
Netlify setup and deployment reference for `/mb-site` legacy paths. Sites use static export (`next build` → `out/` directory) deployed to Netlify.
