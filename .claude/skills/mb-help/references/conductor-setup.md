...
2. **Is the vip path correct?** Check that the path in the script actually contains `.claude/skills/mb-start/SKILL.md`.
...
```bash
ls /absolute/path/to/vip/.claude/skills/mb-start/SKILL.md
```
...
You can. Running `cd ~/Documents/GitHub/my-business && claude` in a regular terminal works perfectly — `/mb-setup` and `/mb-start` handle all the vip linkage automatically.
