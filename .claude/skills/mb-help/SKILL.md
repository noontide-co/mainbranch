...
| What is a repo "role"? | Every repo has a `role` (what it is: `business`/`site`/`offer`/`product`/`finance`/etc.) and a `parent` (what it hangs under). The hub is the root (`role: business`). "Child repo" is a position, not a kind — any role can be a child, and children can have children (hub → offer → that offer's site). Declare it in `.mainbranch/repo.json` (child) or the hub's `core/operations/repo-topology.md`. If `mb status` says "no topology role declared," set one. Run `mb graph --tree` to see the shape. |
