...
| `{{mood}}` | `core/brand/visual-style.md` → mood descriptors |
| `{{primary_hex}}` | `core/brand/visual-style.md` → primary color |
| `{{secondary_hex}}` | `core/brand/visual-style.md` → secondary/accent color |
| `{{prompt_fragments}}` | `core/brand/visual-style.md` → AI image prompt fragments |
...
**Use for:** Creative variation copy lines (from `/mb-ads` hook library output).
